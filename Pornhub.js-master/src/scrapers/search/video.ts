...
export async function videoSearch(engine: Engine, keyword: string, options: VideoSearchOptions): Promise<{
    ...
    // engine.request.get expects a single options object or no options depending on implementation.
    // Pass headers via the engine.request default headers if supported; otherwise call with only the URL.
    // Use the simpler call to avoid TypeScript error about arguments.
