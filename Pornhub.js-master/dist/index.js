...
var Route = {
  ...
  autocomplete(keyword, {
    ...
    return (0, import_urlcat.default)(BASE_URL, "/api/v1/video/search_autocomplete", {
      q: keyword,
   ...
   */
  gifPage(id) {
    return (0, import_urlcat.default)(BASE_URL, "/gif/:id", { id });
  },
  videoPage(id) {
...
// src/scrapers/pages/gif.ts
async function gifPage(engine, urlOrId) {
...
function parseByDom2(html, $) {
  var _a, _b;
  const voteUp = parseReadableNumber($("#votesUp").val() || "0");
  const voteDown = parseReadableNumber($("#votesDown").val() || "0");
  const gifVideo = $("#gifImageSection #js-gifToWebm").first();
  const title = getDataAttribute(gifVideo, "gif-title", "");
  const gif = getDataAttribute(gifVideo, "gif", "");
  const mp4 = getDataAttribute(gifVideo, "mp4", "");
  const webm = getDataAttribute(gifVideo, "webm", "");
...
function parseSourceVideo($) {
  try {
    ...
    const href = getAttribute(videoLink, "href", "");
